...
function DesktopApp() {
  ...
  const [loggedIn, setLoggedIn] = aS(() => !!(localStorage.getItem('tavil_token') || sessionStorage.getItem('tavil_token')));
  const [loginView, setLoginView] = aS('login'); // 'login' | 'register'
  ...
  const handleSignIn = (remember) => {
    const token = 'demo_session_' + Date.now();
    if (remember) localStorage.setItem('tavil_token', token);
    else sessionStorage.setItem('tavil_token', token);
    setLoggedIn(true);
  };
  ...
  if (!loggedIn) {
    return (
      <DesktopAppCtx.Provider value={ctx}>
        {loginView === 'register'
          ? <DRegister onBack={() => setLoginView('login')}/>
          : <DLogin onSignIn={handleSignIn} onGoRegister={() => setLoginView('register')}/>
        }
  ...
  const renderContent = () => {
    switch (screen) {
      ...
      case 'profile': return <DProfile onLogout={handleLogout}/>;
