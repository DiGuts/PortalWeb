...
function DPageTransition({ transitionKey, mode = 'page', direction = 'forward', children }) {
  ...
  tE(() => {
    if (prevKey.current === transitionKey) {
      // Same screen, just refresh content reference (e.g. tweaks update)
    ...
    setLayers(prev => [
      ...
      { key: transitionKey, content: children, phase: 'pre-enter', mode, direction },
    ]);
  ...
  }, [transitionKey, children, mode, direction]);
