...
function DProfile({ onLogout }) {
  ...
  const [editOpen, setEditOpen] = sc2S(false);
  const [pVisible, setPVisible] = sc2S(true);
  ...
  return (
    <div>
      ...
      <div style={{ marginTop: 18 }}>
        ...
        <div style={{ display: 'flex', gap: 24, alignItems: 'flex-end', padding: '0 24px', marginTop: -56, marginBottom: 24 }}>
          ...
          <div style={{ paddingBottom: 12, display: 'flex', gap: 8 }}>
            <DBtn variant="secondary" icon="edit" onClick={() => setEditOpen(true)}>Editar perfil</DBtn>
          </div>
        </div>
      </div>
      ...
      {/* Edit profile modal */}
      <DModal open={editOpen} onClose={() => setEditOpen(false)} title="Editar perfil" width={620}>
        {/* Avatar row */}
        <div style={{
          display: 'flex', alignItems: 'center', gap: 18, padding: '6px 0 18px',
          marginBottom: 18, borderBottom: `1px solid ${theme.border}`
        }}>
          <DAvatar name={user.name} size={72} />
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ fontSize: 14, fontWeight: 600, color: theme.text, marginBottom: 4 }}>Foto de perfil</div>
            <div style={{ fontSize: 12, color: theme.textMuted, marginBottom: 10 }}>JPG o PNG, mínim 256×256 px. Es mostra a tot el portal.</div>
            <div style={{ display: 'flex', gap: 8 }}>
              <DBtn size="sm" variant="secondary" icon="plus">Puja una foto</DBtn>
              <DBtn size="sm" variant="subtle">Elimina</DBtn>
            </div>
          </div>
        </div>

        {/* All data — managed by portal admin */}
        <div style={{
          padding: 14, background: theme.bgAlt, border: `1px solid ${theme.border}`, borderRadius: 10,
          display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14, marginBottom: 18
        }}>
          <div>
            <div style={{ fontSize: 10.5, color: theme.textFaint, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.12em', marginBottom: 4 }}>Nom complet</div>
            <div style={{ fontSize: 13.5, color: theme.text }}>{user.name}</div>
          </div>
          <div>
            <div style={{ fontSize: 10.5, color: theme.textFaint, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.12em', marginBottom: 4 }}>Correu corporatiu</div>
            <div style={{ fontSize: 13.5, color: theme.text }}>marta.vidal@tavil.com</div>
          </div>
          <div>
            <div style={{ fontSize: 10.5, color: theme.textFaint, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.12em', marginBottom: 4 }}>Rol</div>
            <div style={{ fontSize: 13.5, color: theme.text }}>{user.role}</div>
          </div>
          <div>
            <div style={{ fontSize: 10.5, color: theme.textFaint, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.12em', marginBottom: 4 }}>Departament</div>
            <div style={{ fontSize: 13.5, color: theme.text }}>{user.dept}</div>
          </div>
          <div>
            <div style={{ fontSize: 10.5, color: theme.textFaint, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.12em', marginBottom: 4 }}>Telèfon</div>
            <div style={{ fontSize: 13.5, color: theme.text }}>+34 937 20 84 00</div>
          </div>
          <div>
            <div style={{ fontSize: 10.5, color: theme.textFaint, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.12em', marginBottom: 4 }}>Extensió</div>
            <div style={{ fontSize: 13.5, color: theme.text }}>{user.ext}</div>
          </div>
          <div>
            <div style={{ fontSize: 10.5, color: theme.textFaint, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.12em', marginBottom: 4 }}>Oficina</div>
            <div style={{ fontSize: 13.5, color: theme.text }}>Seu central · 2a planta</div>
          </div>
          <div>
            <div style={{ fontSize: 10.5, color: theme.textFaint, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.12em', marginBottom: 4 }}>Incorporació</div>
            <div style={{ fontSize: 13.5, color: theme.text }}>12 setembre 2022</div>
          </div>
          <div style={{ gridColumn: '1 / -1', fontSize: 11.5, color: theme.textFaint, display: 'flex', alignItems: 'center', gap: 6, paddingTop: 4 }}>
            <DIcon name="eye" size={12} /> Totes les dades les gestiona l'administrador del portal.
          </div>
        </div>
        ...
        <label style={{
          ...
          background: theme.bgAlt, border: `1px solid ${theme.border}`, borderRadius: 10, cursor: 'pointer',
          marginTop: 4
        }}>
          <div onClick={() => setPVisible(!pVisible)} style={{
            width: 38, height: 22, borderRadius: 11, background: pVisible ? accent.primary : theme.border,
            position: 'relative', transition: 'background 200ms', cursor: 'pointer', flexShrink: 0
          }}>
            <div style={{
              position: 'absolute', top: 2, left: pVisible ? 18 : 2, width: 18, height: 18, borderRadius: 9,
              background: '#fff', transition: 'left 200ms'
            }} />
          </div>
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: 13.5, color: theme.text, fontWeight: 500 }}>Visible al directori</div>
            <div style={{ fontSize: 11.5, color: theme.textMuted, marginTop: 2 }}>La resta de l'equip et podrà trobar a Qui és qui.</div>
          </div>
        </label>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 20, paddingTop: 16, borderTop: `1px solid ${theme.border}` }}>
          <div style={{ fontSize: 11.5, color: theme.textFaint }}>Els canvis es desen al teu compte TAVIL.</div>
          <div style={{ display: 'flex', gap: 8 }}>
            <DBtn variant="secondary" onClick={() => setEditOpen(false)}>{t.common.cancel}</DBtn>
            <DBtn icon="check" onClick={() => setEditOpen(false)}>Desa els canvis</DBtn>
          </div>
        </div>
      </DModal>
    </div>);
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'center', maxWidth: 440 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24 }}>
            <label style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 13, color: theme.textMuted, cursor: 'pointer' }}>
              <input type="checkbox" style={{ accentColor: accent.primary }} />
          ...
          <DBtn size="lg" full iconRight="arrowRight" onClick={onSignIn}>{t.common.signIn}</DBtn>
          ...
          <DBtn size="lg" full variant="secondary">Continua amb SSO corporatiu</DBtn>
...
Object.assign(window, { DVoice, DRequests, DCampus, DEspai, DProfile, DLogin });