...
export function CreateNoticeModal({ open, onClose, onCreated }: {
  ...
  useEffect(() => {
    if (open) {
      setTitle(''); setContent(''); setLink(''); setLinkText('');
      setKind('warning'); setActive(true); setError(null); setSaving(false);
    }
  }, [open]);
  ...
  return (
    ...
    >
      <AField label="Títol">
        <AInput value={title} onChange={e => setTitle(e.target.value)} placeholder="Títol del banner" />
      </AField>
      <AField label="Contingut" hint="Text breu, visible al portal.">
        <ATextarea rows={3} value={content} onChange={e => setContent(e.target.value)} placeholder="Missatge de l'avís" />
      </AField>
